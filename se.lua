...
function Update:Window(Config)
	...
	local abc = false;
	local currentpage = "";
	local keybind = keybind or Enum.KeyCode.G;
	...
	function uitab:Tab(text, img)
		...
		local main = {};
		function main:Button(text, img, callback)
			...
			ImageLabel.Image = img or "rbxassetid://10734898355";
		...
		function main:Slider(text, min, max, set, callback)
			...
			UserInputService.InputChanged:Connect(function(Input)
				if Dragging and (Input.UserInputType == Enum.UserInputType.MouseMovement or Input.UserInputType == Enum.UserInputType.Touch) then
					...
					local raw = (tonumber(max) - tonumber(min)) / 1000 * bar1.AbsoluteSize.X + tonumber(min)
					-- Value = tonumber(string.format("%.2f", raw)) -- float 2 angka di belakang titik

					Value = math.floor(raw * 1000 + 0.05) / 1000 -- langkah 0.01
					...
					local clamped = math.clamp(Input.Position.X - bar1.AbsolutePosition.X, 0, 100)
					bar1.Size = UDim2.new(0, clamped, 0, 4)
					circlebar.Position = UDim2.new(0, clamped - 5, 0, -5)

				end;
			end);
		end;
		function main:Textbox(text, disappear, default, callback)
